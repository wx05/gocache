...
type Cache struct {
	...
	lock      sync.RWMutex                   //对于ll 和cache操作时需要加锁
	onEvicted func(key string, value string) //对于淘汰的数据处理时的回调函数
}
...
// New 创建lru列表
func New(maxBytes int, onEvicted func(key string, value string)) *Cache {
	return &Cache{
		maxBytes:  maxBytes,
		ll:        list.New(),
...
func (c *Cache) Add(key string, value string) {
	...
	if ele, ok := c.cache[key]; ok {
		...
		ele.Value.(*entry).value = value //更新值
		c.cache[key] = c.ll.Front()      //因为前面已经push到Front了，只需要更新索引即可。

		diff := len(ele.Value.(*entry).value) - len(value) //计算出当前的value的差异
