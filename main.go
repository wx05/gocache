...
func createGroup() *day6.Group {
	return day6.NewGroup("scores", 2<<10, day6.GetterFunc(func(key string) ([]byte, error) {
...
func startCacheServer(addr string, addrs []string, g *day6.Group) {
	peers := day6.NewHTTPPool(addr)
	peers.Set(addrs...)
...
func startAPIServer(apiAddr string, gee *day6.Group) {
...
func main() {
	...
	goCache := createGroup() //创建数据组(现在只有一个写死的scores)
